...
const S = {
  ...
  tabBtn: (active) => ({ padding: "8px 12px", borderRadius: 8, cursor: "pointer", background: active ? "#eef2ff" : "transparent", border: active ? "1px solid #c7d2fe" : "1px solid transparent" }),
  input: { padding: "8px 10px", borderRadius: 8, border: "1px solid #e2e8f0", width: "100%" },
  fieldRow: { display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8, alignItems: "center" },
  assignCard: { border: "1px solid #e6eef8", background: "#fbfdff", padding: 10, borderRadius: 8 }
};
...
export default function Teacher() {
  ...
  // Tabs: keep internal "overview"; for "assignments" route to external page
  const [tab, setTab] = useState("overview"); // "overview" | "assignments"

  // --- Assignments-related states preserved for overview composer (optional)
  const demoAssignments = [
    {
      id: "a1",
      title: "Algebra: Linear Equations (HW)",
  ...
  const [assignments, setAssignments] = useState(teacher.assignments && teacher.assignments.length ? teacher.assignments : demoAssignments);
  const [newTitle, setNewTitle] = useState("");
  const [newClassId, setNewClassId] = useState(teacher.classes[0]?.id || "");
  const [newStudentId, setNewStudentId] = useState("");
  const [newDueDate, setNewDueDate] = useState("");
  const [newDifficulty, setNewDifficulty] = useState("Medium");

  function addAssignment() {
    if (!newTitle.trim()) return alert("Enter an assignment title");
    ...
    const asg = {
      ...
      dueDate: newDueDate || new Date(Date.now() + 1000 * 60 * 60 * 24 * 5).toISOString().slice(0, 10),
      difficulty: newDifficulty,
      status: "open",
      createdAt: new Date().toISOString(),
    };
    setAssignments((s) => [asg, ...s]);
    setNewTitle("");
    setNewStudentId("");
    setNewDueDate("");
    setNewDifficulty("Medium");
    // do not switch to internal assignments view — route instead if user wants to manage there
  }
      ...
      <div style={S.wrap}>
        ...
        <div style={{ marginTop: 12 }}>
          <div style={S.tabs}>
            ...
            {/* NAV tab: route to external assignments page instead of internal pane */}
            <div
              style={S.tabBtn(false)}
              onClick={() => {
                // navigate to your existing assignments route
                navigate("/assignments"); // <-- change path if your assignments page uses a different route
              }}
            >
              Assignments
            </div>

            {/* optional: keep a quick link for managing assignments in a new tab */}
            <div
              style={{ ...S.tabBtn(false), marginLeft: "auto", fontSize: 12, color: "#475569", alignSelf: "center" }}
          ...
          {/* Overview content (unchanged, original UI) */}
          {tab === "overview" && (
            <>
              {/* KPIs */}
              <div style={S.kpi}>
              ...
              {/* Main grid (original content kept) */}
